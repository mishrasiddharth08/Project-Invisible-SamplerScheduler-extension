...
This module fills in the ``comfy.*`` names the vendored sources need so they
can be imported unchanged.  It is *additive*: where Forge Neo already provides
a ``comfy`` package (``modules_forge/packages/comfy``), that package is adopted
and only its missing submodules are supplied, so nothing the host relies on is
shadowed.  It must be imported *before* any of the sibling submodules.
"""
...
def _install_comfy_stubs() -> None:
    """Fill in the ``comfy.*`` names RES4LYF needs, without clobbering Forge.

    Forge Neo ships its own vendored ``comfy`` package at
    ``modules_forge/packages/comfy`` (``comfy.lora``, ``comfy.utils``,
    ``comfy.weight_adapter``) and that directory is on ``sys.path``, so a bare
    ``import comfy`` resolves to the *real* Forge package.  Replacing it with a
    synthetic module would silently break LoRA loading and ControlNet
    conversion the moment anything did a bare ``import comfy``.

    So: adopt the host package if one exists and only *add* the submodules it
    is missing.  Only when there is no host package at all do we synthesise a
    namespace of our own.
    """
    if getattr(sys.modules.get("comfy"), "_pi_samplersched_compat", False):
        return

    def _new_module(name: str) -> types.ModuleType:
        """Return the module ``name``, creating an empty one only if absent.

        Reusing an already-imported module is what keeps this shim additive:
        anything the host (or another extension) has already put under
        ``comfy.*`` survives untouched.
        """
    ...
    # --- adopt Forge's vendored comfy package if it is importable ---------
    ...
    if comfy is None:
        ...
        comfy._pi_synthetic_root = True
    comfy._pi_samplersched_compat = True

    # comfy.utils ---------------------------------------------------------
    # Never replace a host ``comfy.utils``; only top up what is missing.
    utils = getattr(comfy, "utils", None) or sys.modules.get("comfy.utils")
    if utils is None:
        utils = _new_module("comfy.utils")
    if not hasattr(utils, "bislerp"):
    ...
    # setdefault semantics: a host or another extension that already defined
    # these keeps its own version.
    for _n, _v in (
        ("EPS", EPS),
        ("CONST", CONST),

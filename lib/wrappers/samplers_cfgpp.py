...
def sample_euler_cfgpp(model, x, sigmas, extra_args=None, callback=None, disable=None, s_churn=0., s_tmin=0., s_tmax=float('inf'), s_noise=1.):
    ...
    model, extra_args = _capture_unconditional(model, extra_args)
...
def _capture_unconditional(model, extra_args):
    ...
    class Capture:
        ...
        def __setattr__(self, name, value):
            if name in {"init_latent", "mask", "nmask"}:
                setattr(self.real, name, value)
            else:
                object.__setattr__(self, name, value)
        def __delattr__(self, name):
            if name in {"init_latent", "mask", "nmask"}:
                delattr(self.real, name)
            else:
    ...
    def post_cfg(args):
        sigma = args["sigma"]
        sigma = sigma.reshape(sigma.shape + (1,) * (args["input"].ndim - sigma.ndim))
        uncond = args["uncond_denoised"]
        if uncond is None:
            uncond = args["cond_denoised"]
        capture.last_noise_uncond = (args["input"] - uncond) / sigma
        return args["denoised"]
    extra_args = dict(extra_args or {})
    options = dict(extra_args.get("model_options") or {})
    extra_args["model_options"] = set_model_options_post_cfg_function(options, post_cfg, True)
    return capture, extra_args
